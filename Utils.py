...
class utilities:
    ...
    #to detect the color of the tape from hsv image
    def edge_detection(image):
        
        #image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        
        lower_limit = np.array([120,95,0], dtype = "uint8") # lower limit of tape  color
        upper_limit = np.array([138,100,255], dtype = "uint8") # upper limit of tape color
        
        mask = cv2.inRange(image,lower_limit,upper_limit) # this mask will filter out everything but blue

        # canny detect edges
        edge_image = cv2.Canny(mask, 50, 100) 
        return edge_image
    
    def region_of_interest(image,height,width):
         # extract the height and width of the edges frame
        mask = np.zeros_like(image) # make an empty matrix with same dimensions of the edges frame
    ...
    def average_slope_intercept(frame, line_segments):
        ...
        if len(left_fit) > 0:
            lane_lines.append(mp.make_points(frame, left_fit_average))

        right_fit_average = np.average(right_fit, axis=0)
        if len(right_fit) > 0:
            lane_lines.append(mp.make_points(frame, right_fit_average))
